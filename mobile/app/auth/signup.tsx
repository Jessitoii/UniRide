...
const SignupScreen = () => {
  ...
  const handleOpenPrivacyPolicy = () => {
    Linking.openURL('https://uniride-7g6w.onrender.com/privacy-policy.html');
  };
  ...
  const renderStep2 = () => (
    <View style={styles(theme).stepContainer}>
      ...
      <DropDownPicker
        ...
        items={universities.map((uni, index) => ({
          label: uni.name.tr,
          value: uni.name.tr,
          key: `${uni.name.tr}-${uni.name.tr}-${index}`
        }))}
      ...
      <DropDownPicker
        ...
        items={
          universities.find((uni) => uni.name.tr === university || uni.name.en === university)?.faculties.map((faculty, index) => ({
            label: faculty.name.tr,
            value: faculty.name.tr,
            key: `${university}-${faculty.name.tr}-${index}`
          })) || []
        }
        setOpen={(open) => {
          setFacultyOpen(open);
