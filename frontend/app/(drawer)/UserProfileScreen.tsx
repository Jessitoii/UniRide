...
import { Review, Post } from '@/components';
...
export default function UserProfileScreen() {
  ...
  // Support both id (legacy/standard) and userId (from DriverCard/Post)
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setProfile(data);
          // Set reviews from the profile data directly
          if (data.reviewReceived) {
            setReviews(data.reviewReceived);
          }
        } else {
      ...
      <ScrollView contentContainerStyle={styles(theme).scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles(theme).profileHeader}>
          ...
          <View style={styles(theme).avatarContainer}>
            <Avatar
              user={{ id: profile.id, hasCustomPhoto: profile.hasCustomPhoto }}
              size={100}
        ...
        {/* Car Info / Driver Card */}
        {profile.car && (
          <View style={styles(theme).sectionContainer}>
            <Text style={styles(theme).sectionTitle}>{t('vehicle')}</Text>
            <DriverCard
              id={profile.id}
              name={profile.name}
              ...
              vehicle={{
                ...
                photoUrl: (getVehicleSource({ brand: profile.car.brand, model: profile.car.model, photoPath: profile.car.photoPath }) as any)?.uri
              }}
              isAvailable={true} // Logic to be determined
            />
          </View>
        )}
              ...
              {reviews.map((review, index) => (
                <View key={index} style={{ width: 280, marginRight: 12 }}>
                  <Review
                    ...
                    hasCustomPhoto={review.user.hasCustomPhoto}
                  />
                </View>
              ))}
            </ScrollView>
