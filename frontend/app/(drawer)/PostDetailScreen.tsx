...
import { View, Text, StyleSheet, Image, ScrollView, TouchableOpacity, ActivityIndicator, StatusBar, useColorScheme, Alert, Modal } from 'react-native';
import { useRouter, useLocalSearchParams } from 'expo-router';
import { BASE_URL } from '@/env';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { Post, InterestedUser } from '@/components';
import DriverCard from '@/components/business/DriverCard';
import { getVehicleSource } from '@/utils/fetchProfilePhoto';
...
export default function PostDetailScreen() {
  ...
  const isExpired = postDate < new Date() || post.status === 'COMPLETED';
      ...
      {post.interestedUsers?.length ? (
        post.interestedUsers.map((interestedUser: any) => (
          <InterestedUser
            ...
            hasCustomPhoto={interestedUser.user.hasCustomPhoto}
            matchedUserId={post.matchedUserId}
            onMatchPress={isExpired ? undefined : handleMatchUser}
          />
        ))
      ) : (
    ...
    return (
      <View style={styles(theme).sectionContainer}>
        ...
        <DriverCard
          id={post.user.id}
          name={post.user.name}
          rating={post.user.stars || 5.0} // Fallback
          hasCustomPhoto={post.user.hasCustomPhoto}
          vehicle={post.user.car ? {
            brand: post.user.car.brand,
            model: post.user.car.model,
            color: post.user.car.color,
            plateNumber: post.user.car.plateNumber,
          } : undefined}
          isAvailable={true} // Driver of the post is 'available' in this context
          onPress={() => { /* Navigate to driver profile if needed */ }}
          onMessagePress={() => { /* Start chat logic */ }}
          onCallPress={() => { /* Call logic */ }}
        />
      </View>
    );
  };
  ...
  return (
    <View style={styles(theme).container}>
      <StatusBar barStyle={theme.isDark ? "light-content" : "dark-content"} backgroundColor="transparent" />
      ...
      <ScrollView contentContainerStyle={styles(theme).scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* Expired / Matched Banners */}
        {post.matchedUserId ? (
          <View style={[styles(theme).statusBanner, { backgroundColor: theme.colors.success + '20' }]}>
            <MaterialIcons name="check-circle" size={20} color={theme.colors.success} />
            <Text style={[styles(theme).statusText, { color: theme.colors.success }]}>
              {t('matched_with')}
            </Text>
          </View>
        ) : isExpired ? (
          <View style={[styles(theme).statusBanner, { backgroundColor: theme.colors.textLight + '20' }]}>
            <MaterialIcons name="event-busy" size={20} color={theme.colors.textLight} />
            <Text style={[styles(theme).statusText, { color: theme.colors.textLight }]}>
              {t('post_expired')}
            </Text>
          </View>
        ) : null}
      ...
      <View style={styles(theme).backButtonContainer}>
        <TouchableOpacity
          onPress={() => router.back()}
          activeOpacity={0.8}
          style={styles(theme).backButtonTouch}
        >
          <BlurView intensity={30} tint={theme.isDark ? 'dark' : 'light'} style={styles(theme).backButtonBlur}>
        ...
        <View style={styles(theme).bottomBar}>
          {!post.matchedUserId ? (
            post.interestedUsers?.some((user: any) => user.user.id === profile.id) ? (
              ...
              <View style={[styles(theme).actionButton, { backgroundColor: theme.colors.surface, borderWidth: 1, borderColor: theme.colors.border }]}>
                <Text style={[styles(theme).actionButtonText, { color: theme.colors.textLight }]}>{t('post_expired')}</Text>
              </View>
            ) : (
              <TouchableOpacity
