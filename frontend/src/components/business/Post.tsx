...
const Post: React.FC<PostProps> = ({ id, from, to, userName, date, startTime, endTime, route, userId, userLocation, onPress, stars, matchedUserId, interestedUsers }) => {
        ...
        <View style={styles(theme).headerSection}>
          ...
          {matchedUserId ? (
            <View style={[
              ...
              matchedUserId === userId ? styles(theme).statusBadgeConfirmed : styles(theme).statusBadgeReserved
            ]}>
              <Text style={[
                styles(theme).statusText,
                matchedUserId === userId ? styles(theme).statusTextConfirmed : styles(theme).statusTextReserved
              ]}>
                {matchedUserId === userId ? "Eşleşildi" : "Dolu"}
              </Text>
              {matchedUserId === userId && <MaterialIcons name="check" size={12} color={theme.colors.white} />}
            </View>
          ) : (interestedUsers && interestedUsers.includes(userId)) ? (
            <View style={styles(theme).statusBadgePending}>
              <Text style={styles(theme).statusTextPending}>Talep Gönderildi</Text>
            </View>
          ) : null}
...
const styles = (theme: ThemeType) => StyleSheet.create({
  ...
  statusBadgePending: {
    ...
    paddingVertical: 4,
    borderRadius: theme.borderRadius.full,
    backgroundColor: 'pink' // Placeholder, will refine with theme if available or direct hex
  },
  statusTextPending: {
    fontSize: theme.typography.fontSize.xs,
    fontFamily: theme.typography.fontFamily.bold,
    color: '#C2185B' // Pink darken
  }
});
